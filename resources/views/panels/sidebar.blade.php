<aside class="{{ $configData['sidenavMain'] }} @if (!empty($configData['activeMenuType'])) {{ $configData['activeMenuType'] }} @else {{ $configData['activeMenuTypeClass'] }} @endif
    {{ $configData['sidenavMainColor'] }}">
            ...
            <a class="brand-logo darken-1" href="{{ asset('/') }}">
                @if (!empty($configData['mainLayoutType']) && isset($configData['mainLayoutType']))
                    ...
                    <img class="show-on-medium-and-down hide-on-med-and-up"
                        src="{{ asset($configData['smallScreenLogo']) }}" alt="logo" />
                @endif
                ...
                @else
                    ...
                    <li class="bold {{ request()->is($menu->url . '*') ? 'active' : '' }}">
                        <a class="{{ $custom_classes }} {{ request()->is($menu->url . '*') ? 'active ' . $configData['activeMenuColor'] : '' }}"
                            ...
                            href="   @if ($menu->url ===
                            'javascript:void(0)'){{ $menu->url }} @else{{ url($menu->url) }} @endif"
